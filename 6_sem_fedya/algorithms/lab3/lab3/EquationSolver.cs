...
namespace lab3
{
    public class EquationSolver
    {
        ...
        {
            var individuals = InitializePopulation();
            var solution = GetSolution(individuals);
        ...
        {
            return individuals.Where(_ => Probability.GenerateProbability() > SelectionProbabilityBound).ToList();
        }
        ...
        private List<Individual> Crossbreeding(List<Individual> individuals)
        {
            ...
            for (var i = 0; i < individuals.Count - 1; i += 2)
            {
        ...
        {
            var nextIndividuals = Mutation(Crossbreeding(Select(prevIndividuals)));
            var sortedPrevIndividuals = prevIndividuals.OrderBy(GetSolutionError).ToList();
        ...
        {
            var strongestIndividual = individuals.OrderBy(GetSolutionError).ToList().First();
