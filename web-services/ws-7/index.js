...
app.post('/events', async (req, res) => {
    try {
        let { aggregateType, eventType, payload } = req.body;
        const aggregateId = req.query.aggregateId ?? uuidv4();
        ...
        try {
            ...
            await client.query(`
                ...
                VALUES ($1, $2, $3) ON CONFLICT (aggregate_id) DO
                UPDATE SET
                    data = EXCLUDED.data,
                    created_at = NOW()
...
app.get('/instances', async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT *
            FROM snapshots
            WHERE aggregate_type = 'client'
            ORDER BY created_at DESC
        `);

        if (result.rowCount > 0) {
            const snapshots = result.rows;
            res.send(snapshots.map((snapshot) => ({
                aggregateId: snapshot.aggregate_id,
                aggregateType: snapshot.aggregate_type,
                data: snapshot.data,
            })))
...
app.get('/instances/:aggregateId', async (req, res) => {
    try {
